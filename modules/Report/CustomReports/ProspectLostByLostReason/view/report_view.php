		...
		<div class="col-md-12">
			<div class="datadisplay-outer">
				...
				<table cellpadding="0" cellspacing="0" border="0" class="datadisplay" id="sqcrmlist">
					...
					<?php
					if ($prospect_lost->getNumRows() > 0) {
						while ($prospect_lost->next()) {
							echo '<tr>' ;
							foreach ($fields_info as $fields=>$info) {
								$fieldobject = 'FieldType'.$info["field_type"];
								$val = $do_crm_fields->display_field_value($prospect_lost->$fields,$info["field_type"],$fieldobject,$prospect_lost,5,false) ;
...
$(document).ready(function() {  
	...
	<?php
	if (count($prospect_lost_data_by_amount["data"]) > 0) {
	?>
