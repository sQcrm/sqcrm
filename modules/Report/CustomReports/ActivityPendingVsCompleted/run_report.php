...
*/
include_once(BASE_PATH.'/modules/Report/CustomReports/ActivityPendingVsCompleted/ActivityPendingVsCompleted.class.php');
$activity = new ActivityPendingVsCompleted();
...
$held = 'Held' ;
$planned = 'Planned' ;
if (is_object($activity->parse_config) && property_exists($activity->parse_config,'fieldValueMaps')) {
	if (property_exists($activity->parse_config->fieldValueMaps,'event_status')) {
		if (property_exists($activity->parse_config->fieldValueMaps->event_status,'Planned')) {
			$planned = $activity->parse_config->fieldValueMaps->event_status->Planned ;
		}
		if (property_exists($activity->parse_config->fieldValueMaps->event_status,'Held')) {
			$held = $activity->parse_config->fieldValueMaps->event_status->Held ;
		}
	}
}
...
$activities_by_type = $activity->get_activity_pending_vs_completed($where);
