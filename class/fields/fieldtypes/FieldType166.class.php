...
/**
* Class FieldType166
* Field Type 166 : Product Selector 
* @author Abhik Chakraborty
*/

class FieldType166 extends CRMFields {
	public $table = "fields";
	...
	*/
	public static function display_field($name,$value = '',$css = '') {
		...
		if ((int)$value > 0) {
			...
			$object->query("select ".$retrun_fields." from `products` where `idproducts` = ?",array($value));
		...
		if ($special_field === true) {
			$html .=' var href = \'/popups/listdata_popup_modal?m=Products&action=list&special_field=yes&special_field_name='.$name_explode[1].'&fielddisp=products&fieldname='.$name.'\';';
		} else {
			$html .=' var href = \'/popups/listdata_popup_modal?m=Products&action=list&fielddisp=products&fieldname='.$name.'\';';
		}
	...
	*/
	public static function display_value($value,$product_name='',$linked=true) {
		if ($product_name != '') {
			if ($linked === true)
				return '<a href="'.NavigationControl::getNavigationLink('Products',"detail",$value).'">'.$product_name.'</a>';
			else
				return $product_name;
		} else {
			...
			if ($display_val == '') {
				...
				if ($linked === true)
					return '<a href="'.NavigationControl::getNavigationLink('Products',"detail",$value).'">'.$display_val.'</a>';
				else
					return $display_val;
			}
		}
