...
/**
* Class FieldType143
* Field Type 143 : Contacts Selector and copy address.
* @author Abhik Chakraborty
*/

class FieldType143 extends CRMFields {
	public $table = "fields";
	...
	*/
	public static function get_field_type() {
		return _('Contact Selector and copy address.') ;
	}
	...
	*/
	public static function display_field($name,$target_module,$value = '',$css = 'form-control input-sm') {
		$html = '';  
		$display_val = '';
		if ((int)$value > 0) {
			$object = new Contacts();
		...
		if ($special_field === true) {
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Contacts&action=list&fielddisp=contact&special_field=yes&special_field_name='.$name_explode[1].'&fieldname='.$name.'\';';
		} else {
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Contacts&action=list&copy_cnt_address=yes&fielddisp=contact&target_module='.$target_module.'&fieldname='.$name.'\';';
		}
		$html .=  
		"\n".' 
		if (href.indexOf(\'#\') == 0) {
