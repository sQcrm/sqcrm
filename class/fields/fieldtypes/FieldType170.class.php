...
/**
* Class FieldType170
* Field Type 170 : Quotes Selector 
* @author Abhik Chakraborty
*/

class FieldType170 extends CRMFields {
	public $table = "fields";
	...
	public static function display_field($name,$value = '',$css = 'form-control input-sm') {
		...
		if ((int)$value > 0) {
			...
			if (preg_match("/:::/", $name)) {
				...
				$name = 'quotes_'.$name_explode[1];
				$special_field = true ;
			}
			$html .='<input type="text" class = "'.$css.'" name="quotes_'.$name.'" id="quotes_'.$name.'" value="'.$display_val.'" readonly>';
			...
			if ($special_field === true) {
				$html .= 'var href = \'/popups/listdata_popup_modal?m=Quotes&action=list&fielddisp=quotes&special_field=yes&special_field_name='.$name_explode[1].'&fieldname='.$name.'\';';
			} else {
				$html .= 'var href = \'/popups/listdata_popup_modal?m=Quotes&action=list&fielddisp=quotes&fieldname='.$name.'\';';
			}
			$html .=  
			"\n".' 
			if (href.indexOf(\'#\') == 0) {
	...
	*/
	public static function display_value($value,$quote_name='',$linked=true) {
		if ($quote_name != '') {
			if ($linked === true)
				return '<a href="'.NavigationControl::getNavigationLink('Quotes',"detail",$value).'">'.$quote_name.'</a>';
			else
				return $quote_name;
		} else {
			...
			if ($display_val == '') {
				...
				if ($linked === true)
					return '<a href="'.NavigationControl::getNavigationLink('Quotes',"detail",$value).'">'.$display_val.'</a>';
				else
					return $display_val ;
			}
		}
