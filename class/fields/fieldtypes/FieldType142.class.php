...
/**
* Class FieldType142
* Field Type 142 : Contacts Selector depending on selected organization
* @author Abhik Chakraborty
*/

class FieldType142 extends CRMFields {
	public $table = "fields";
	...
	*/
	public static function get_field_type() {
		return _('Contact selector for already selected organization') ;
	}
	...
	*/
	public static function display_field($name,$value = '',$css = 'form-control input-sm') {
		...
		if (preg_match("/:::/", $name)) {
			...
			$name = 'contact_'.$name_explode[1];
			$special_field = true ;
		}
		$html .='<input type="text" class = "'.$css.'" name="contact_'.$name.'" id="contact_'.$name.'" value="'.$display_val.'" readonly>';
		...
		'$(\'#select_'.$name.'\').click(function(){ 
			var idorganization_selected = $(\'#idorganization\').val();
			if(idorganization_selected == \'\' || idorganization_selected == \'0\'){
				display_js_error(\'Please select an organization first\',\'js_errors\');
			  return false;
			}
		';
		if ($special_field === true) {
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Contacts&action=list&org_dependent=yes&fielddisp=contact&special_field=yes&special_field_name='.$name_explode[1].'&fieldname='.$name.'\';';
		} else {
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Contacts&action=list&org_dependent=yes&fielddisp=contact&fieldname='.$name.'\';';
		}
		$html .=  
		"\n".' 
		href = href+\'&idorganization=\'+idorganization_selected;
		if (href.indexOf(\'#\') == 0) {
				$(href).modal(\'open\');
		} else {
