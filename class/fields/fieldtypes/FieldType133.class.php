...
/**
* Class FieldType133
* Field Type 133 : Potentials Selector 
* @author Abhik Chakraborty
*/

class FieldType133 extends CRMFields {
	public $table = "fields";
	...
	*/
	public static function display_field($name,$value = '',$css = 'form-control input-sm') {
		...
		if (preg_match("/:::/", $name)) {
			...
			$name = 'potential_'.$name_explode[1];
			$special_field = true ;
		}
		$html .='<input type="text" class = "'.$css.'" name="potential_'.$name.'" id="potential_'.$name.'" value="'.$display_val.'" readonly>';
		...
		if($special_field === true){
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Potentials&action=list&fielddisp=potential&special_field=yes&special_field_name='.$name_explode[1].'&fieldname='.$name.'\';';
		}else{
			$html .= 'var href = \'/popups/listdata_popup_modal?m=Potentials&action=list&fielddisp=potential&fieldname='.$name.'\';';
		}
		$html .=  
		"\n".' 
		if (href.indexOf(\'#\') == 0) {
	...
	* @param string $value
	* @param string $pot_name
	* @param boolean $linked
	*/
	public static function display_value($value,$pot_name='',$linked=true) {
		if ($pot_name != '') {
			if($linked === true)
				return '<a href="'.NavigationControl::getNavigationLink('Potentials',"detail",$value).'">'.$pot_name.'</a>';
			else
				return $pot_name;
		} else {
			...
			if ($display_val == '') {
				...
				if($linked === true)
					return '<a href="'.NavigationControl::getNavigationLink('Potentials',"detail",$value).'">'.$display_val.'</a>';
				else
					return $display_val ;
			}
		}
